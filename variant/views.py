...
from .models import Product,Color,Variant,VariantImage
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if Variant.objects.filter(product=product_obj, color=color_obj).exists():
                messages.error(request, "Variant with the same product and color already exists!")
                return redirect('product_variant')
            ...
            messages.error(request, "Invalid product or color selected!")
            return redirect('product_variant')
